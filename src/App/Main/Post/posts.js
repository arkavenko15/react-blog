const posts=
[
    {
        id:1,
        image:"react-blog/images/image-1.jpg",
        categoria: "HEALTH",
        title: "Stay focus and safe",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:2,
        image:"react-blog/images/image-2.jpg",
        categoria: "TECHNOLOGIES",
        title: "Digest from IT-Company",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:3,
        image:"react-blog/images/image-3.jpg",
        categoria: "HEALTH",
        title: "All you need to know about Human's being",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:4,
        image:"react-blog/images/image-1.jpg",
        categoria: "TECHNOLOGIES",
        title: "Hot Vacancies",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:5,
        image:"react-blog/images/image-1.jpg",
        categoria: "LIFE STYLE",
        title: "Make Life easier with yoga",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:6,
        image:"images/image-1.jpg",
        categoria: "MUSIC",
        title: "System of a Down. Tour 2021",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:7,
        image:"images/image-1.jpg",
        categoria: "EDUCATION",
        title: "Education system in Ukraine",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:8,
        image:"images/image-1.jpg",
        categoria: "TRAVEL",
        title: "Traveling witn friends",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:9,
        image:"images/image-1.jpg",
        categoria: "WEB DEVELOPMENT",
        title: "Web Development with professionals",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
    {
        id:10,
        image:"images/image-1.jpg",
        categoria: "DESIGN",
        title: "All about Design",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
        
    },
    { 
        id:11,
        image:"images/image-1.jpg",
        categoria: "DESIGN",
        title: "Design Projests. Preview",
        author: "JHODE DOE",
        date: "SEPTEMBER 11 2015.",
        fullDescription: `Many marketers made claims about video being the hottest content trend of 2017. There’s so much room for creativity, so many ways to apply it to your industry that you simply have to try integrating video into your feed.
        Offer your fans something valuable in the form of educational or entertainment videos. Spark some conversation by showing something they haven’t seen before, give them an opportunity to walk away having learned something.        
        The videos you share can be educational, entertaining, promotional, shown as stories or just interesting snips. Whatever type of video you choose, make sure you’re staying in your niche and sharing something relevant.`,
        description: "This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non  mauris vitae erat consequat auctor eu in elit. className aptent taciti sociosqu ad litora torquent per conubia nostra."
    },
]

export const getPostObject = (array) => array.reduce((obj,post) => ({
    ...obj,
    [post.id]:post
}),{})
export default posts